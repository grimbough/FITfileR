...
The names of these files indicate the manufacture and model name of the device it was recorded on, as well as the activity type.  There are many other differences (e.g. sensors connected, recording frequency, software versions) that are not encapsulated in the file names.
...
In this case this is a single message that reports power and heart rate thresholds that were set on the device.  One could imagine using these in conjunction with the "records" to measure how well the rider performed relative to the pre-set threshold for this particular activity.
...
```{r, message = FALSE, out.width="100%", warning = FALSE}
library(zoo)
