...
Before we can plot locations need to process the values held in the fit file into longitude and latitude.  First, we remove any entries recorded when satellite reception was lost; these have a value of `NA` in this file, but appear as `2^31-1` in the data from the TomTom so we remove both.  We then convert to longitude and latitude coordinates.
...
library(ggplot2)
ggplot() + 
    geom_line(data = garmin$record, aes(x = timestamp, y = heart_rate), col = 1) + 
    geom_line(data = tomtom$record, aes(x = timestamp, y = heart_rate), col = 2)
```
