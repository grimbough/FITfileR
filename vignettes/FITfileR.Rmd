...
```{r, plot-power-arc, eval = TRUE, fig.cap="Power balance between pedal and the average power delivery arcs through the pedal stroke", fig.width=6, fig.height=3}
library(ggforce)
## create arc plot for each side
ggplot() +
  ## background circle
  geom_circle(data = power, aes(x0 = 0, y0 = 0, r = 1),
           lwd = 9.8, color = "#023047") +
  ...
  geom_point(data = data.frame(x = c(0,0.8,0,-0.8), y = c(0.8, 0, -0.8, 0)),
    ...
    geom_circle(data = power, aes(x0 = 0, y0 = 0, r = 0.8),
           lwd = 2, color = "#023047", n= 8) +
  ## pedal balance text
