...
We're again going to use the FIT file recorded during a Zwift ride on the turbo trainer as the bike had power pedal attached.
...
It takes some data wrangling with the **tidyr** packge to get the power data into a tidy format for **ggplot2**.  Here we also convert the angles from degress as they're provided by **FITfileR** into radians, as this will be useful for the plotting step next.

We also seperatly extract the left/right pedal balance data, which can also be found in the *session* information.
...
```{r, plot-power-arc, eval = TRUE, fig.cap="Power balance between pedal and the average power delivery arcs through the pedal stroke", fig.width=8, fig.height=4}
library(ggforce)
## create arc plot for each side
ggplot() +
  ## background circle
  geom_arc(data = power, aes(x0 = 0, y0 = 0, r = 1, start = 0, end = 2*pi)) +
  ...
  geom_text(data = balance, 
            mapping = aes(x = 0, y = 0, label = balance), size = 10) +
