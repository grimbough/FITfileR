...
```{r, eval = FALSE}
messages <- read.xlsx(system.file("extdata/Profile_gsheets_Aug19.xlsx", package = "fitFileR"), 
                      sheet = 2)
rm.idx <- which(is.na(messages[,1]) & is.na(messages[,2]) & is.na(messages[,3]))
messages <- messages[-rm.idx,c(1:4,7)]
names(messages) <- c("message_type", "key", "value", "type", "scale")
messages <- mutate(messages, type = as.factor(type), scale = as.numeric(scale))
...
fit_message_types <- split(messages[,2:5], messages[[1]])
```

Here we read the data type definitions:

```{r, eval = FALSE}
data_types <- read.xlsx(system.file("extdata/Profile_gsheets_Aug19.xlsx", package = "fitFileR"), 
                             sheet = 1)[,c(1,4,3,2)]
